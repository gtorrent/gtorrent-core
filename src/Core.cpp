...
string gt::Core::getDefaultSavePath() {
  ...
  return savepath == NULL ? string("") : string(savedrive)
...
shared_ptr<gt::Torrent> gt::Core::addTorrent(string path) {
  if (path.empty())
    // Use default constructor instead of null
    return shared_ptr<gt::Torrent>();
  shared_ptr<gt::Torrent> t;
  try {
    t = make_shared<gt::Torrent>(path);
    // pointer necessary to catch exception as a shared ptr would
    // go out of scope
  }
  catch (int exception) {
    return shared_ptr<gt::Torrent>();
  ...
  libtorrent::torrent_handle h =
    m_session.add_torrent(t->getTorrentParams(), ec);
  if (ec.value() != 0) {
    gt::Log::Debug(ec.message().c_str());
    return shared_ptr<gt::Torrent>();
  } else {
  ...
  // TODO(general): Event polling for magnet update information
