...
shared_ptr<Torrent> gt::Core::addTorrent(string path)
{
	...
	try
	{
		t = make_shared<Torrent>(path); //pointer necessary to catch exception as a shared ptr would go out of scope
	}
	catch (int exception)
	{
		return shared_ptr<Torrent>();//Return null if invalid torrent to be handled by GtkTorrentTreeView
	}
	libtorrent::error_code ec;
